...
func LumberjackWriteSyncer() zapcore.WriteSyncer {
	...
	lumberJackLogger := &lumberjack.Logger{
		Filename:   path.Join(globalInstance.BaseConfig.ZapLog.Director, "system.logger."+logName),
		MaxSize:    1,
		MaxBackups: 5,
		MaxAge:     30,
	...
	if globalInstance.BaseConfig.ZapLog.LogInConsole {
...
func RotateWriteSyncer() zapcore.WriteSyncer {
	// 创建日志目录
	logDir := globalInstance.BaseConfig.ZapLog.Director
	...
	rotateLogsLogger, _ := rotatelogs.New(
		...
		rotatelogs.WithLinkName(globalInstance.BaseConfig.ZapLog.LinkName),
